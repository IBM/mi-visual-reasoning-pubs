...
\begin{table}[]
	\caption{Percentage decrease of the number of parameters by units in the MAC Cell between MAC and Simplified-MAC models}
	\centering
	\begin{tabular}{ccccCcCc}
		...
		\cmidrule{2-4} \cmidrule{5-6} \cmidrule{7-8}
	    Simplified-MAC number of parameters  & 263,168  & 262,656       &    6,560,768 \\
	    \cmidrule{2-4} \cmidrule{5-6} \cmidrule{7-8}
...
As a result we could decrease the number of trainable parameters. Table 1 shows a significant decrease in the number of parameters in the Read and Write units as well as a small drop in the Control unit. Those three units combined together compose the MAC cell which is the core of the network. It is important to emphasize that the MAC cell is the recurrent part of the network as opposed to the Input and Output unit that are call only once. That is why this reduction in the number of parameters is particularly important for the three MAC cell units and why we decided to focus on simplifying those equations.