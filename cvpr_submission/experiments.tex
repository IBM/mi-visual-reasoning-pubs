...
For the groups at the lowest level, we chose the following task classes to be placed in those groups:
\begin{description}
	\item[Basic:] \textit{ExistShape}, \textit{GetShape},\textit{GetColor}, \textit{ExistColor}, \textit{Exist}
	\item[Compare Obj-Attr:] \emph{SimpleCompareColor}, \textit{SimpleCompareShape}, \textit{AndSimpleCompareShape}, 
	\textit{AndSimpleCompareColor};
	\item[Compare 2 Obj:] \textit{CompareColor}, \textit{CompareShape},  \textit{AndCompareColor}, \textit{AndCompareShape}, 
	\textit{ExistColorOf}, \textit{ExistShapeOf};
	\item[Spatial:] \textit{ExistSpace}, \textit{ExistColorSpace}, \textit{ExistShapeSpace}, \textit{GetColorSpace}, \textit{GetShapeSpace};
	\item[Cognitive:] \textit{ExistLastColorSameShape}, \textit{ExistLastShapeSameColor}, \textit{ExistLastObjectSameObjectdescription}
\end{description}


We then conducted the following experiments using the Canonical variant of the COG dataset, to study 
whether transfer learning was effective in leveraging information gained by training a group at a higher level
of the hierarchy to learning the constituent task groups below that group at the leaf level.
 \begin{itemize}
	\item Train and test SAMNet on a single task group $t$ for the 5 groups at the lowest level of the hierarchy. 
	These 5 experiments fit into the traditional ML setup of single-task learning;
	\item Train on \textbf{Group A} and test separately on \textbf{Basic}, \textbf{Compare Obj-Attr} and \textbf{Compare 2 Obj};
	...
	can be viewed as training on \textbf{All} and testing on each group at the lowest level separately.
\end{itemize}
