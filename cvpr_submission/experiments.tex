...
First notice that for each of the \textbf{Basic} and \textbf{Obj-Attr} task families, the accuracy is near-perfect in all cases 
suggesting that each contains the most primitive of tasks and therefore do not benefit from training with other task families.
...
to 97.0\% while training on all samples. To furthe rdrill down on this behavior, notice that 
just joining \textbf{Compare} with \textbf{Obj-Attr} and \textbf{Basic} already causes a significant accuracy jump to 92.3\%. 
In hindsight, this is not surprising because the questions in \textbf{Compare} are composed
of fragments in the questions given by \textbf{Basic} and \textbf{Obj-Attr}, and therefore can leverage the reasoning strategies developed there to reason about questions in \textbf{Compare}.
...
is a slight loss incurred by including everything. As seen in the figure, just jointly training with \textbf{Spatial} alone is sufficient to get a boost in the accuracy (97.1\%). To summarize, while joint training helps, one needs to determine how much of correlation is present with the other tasks.
...
the spheres can take any color. This is summarized in~\cref{tab:cogent_conditions}. 
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.4\columnwidth]{img/results/CoGenT_B_results.pdf}
