...
We illustrate the key reasoning steps taken by the model with the example presented in \cref{fig:example}.
First let us analyze, independently of SAMNet, what are the key operations that need to be performed in order to provide the answers. 
As the question concerns \textbf{latest} in the \textbf{Frame 1}, one should look for both \textbf{u} and \textbf{circle} objects and look at their colors (different), thus provide answer: \textbf{false}.
Next one should also memorize the presence of \textbf{pink circle} and move to next Frame.
Analogically, in \textbf{Frame 2} there are both objects present, both with different colors, so the answer is once again \textbf{false}.
Besides, from that point we do not care anymore about the \textbf{pink circle} and remember the presence of \textbf{yellow circle} instead.
This is of utmost importance for the following two frames, as there are no circles down there at all.
So when analyzing \textbf{Frames 3} and \textbf{4} one need to recall the \textbf{yellow circle} from the memory and compare its color with the color of \textbf{u}'s -- as in both cases they are yellow, thus  both answers should be \textbf{true}.
...
As full step-by-step analysis would significantly exceed the page limits, we have cherry picked three steps of reasoning performed by our SAMNet model.
Let us start from \textbf{1}-st reasoning step on Frame \textbf{2}, as visualized in \cref{fig:frame-2-step-1}.
First thing worth noting is that there is already an object stored in memory address 1 (write head is always pointing at "free memory slot", in this case the 2nd slot seems to be partially "polluted" by the object from slot 1).
Second, the attention over both the question and image is clearly focusing over \textbf{u} -- despite we have never trained the system with "word \textbf{u}"-"visual object \textbf{u}" pairs, the SAM Network managed to develop visual grounding on its own.
Third, the time context is clearly focused on \text{Now} -- once again, during the training we have never indicated that the system should pay attention to this (or any other) particular word and the system learned to catch that context on its own (i.e. learned to use the provided mechanisms in the correct way).
...
Next let us analyze \textbf{6}-th reasoning step presented~\cref{fig:frame-2-step-6}.
At that point the system focuses its attention on the second object important from the point of view of the question: \textbf{yellow circle}.
Analogically, the question and visual attention are almost perfectly aligned with the word and object, and, besides, the system caught that the time context of the object is \textbf{Latest}.
Additionally, the "Add New" gate is on, the memory address 2 has a different content and write head moved to the next address -- sadly in here we also observe the "pollution" of addresses 3 and 4.
But despite that, those are still clear indicators that the system has stored the \textbf{yellow circle} in the external memory.
...
In the \textbf{1}-st reasoning step (\cref{fig:frame-4-step-1}) we can once again that  system correctly grounded visual object and temporal context.
