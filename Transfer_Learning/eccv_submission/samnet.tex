...
Similar to the memory, we view $\vec{F}_k$ as a content-addressable memory with 
...
\noindent\textbf{Retrieval Units.} The visual retrieval unit uses the information generated above to extract an attention vector $\vec{va}_t \in \Reals^{H \times W}$ over the feature vectors in $\vec{F}_k$ and a frame object 
$\vec{vo}_t \in \Reals^d$.
