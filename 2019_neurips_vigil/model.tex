...
The visual (resp. memory) retrieval unit uses the information generated above to extract a relevant object $vo_t$ from the frame (resp. $mo_t$ from memory)
...
Let $\whead'_{t-1}$ denote the circular shift to the right of $\whead_{t-1}$ which corresponds to the soft version of this operation.
Then we update the write head according to the equation:
\[ \whead_t = \doadd \cdot \whead'_{t-1} + (1-\doadd) \cdot \whead_{t-1} \]

\noindent{\textbf{Summary Update Unit.}}
%\paragraph{Summary Update Unit.}
This unit updates the summary object to be the outcome of the $t$-th reasoning step.
...
Note that $ro_t$ could be a null object (i.e. 0 vector) in case neither of the gates evaluate to true.
