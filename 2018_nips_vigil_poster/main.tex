...
\begin{itemize}
	\compresslist
	\item[$\bullet$] We introduce a \emph{\textbf{simplified}} variant of the MAC model (\textit{Hudson and Manning, ICLR 2018}), which achieves comparable accuracy while training \emph{\textbf{faster}}.
	\item[$\bullet$] We evaluate both models on CLEVR \& CoGenT, and show that, \emph{\textbf{transfer learning with fine-tuning}} results in a 15 point increase in accuracy, matching the state of the art.
	\item[$\bullet$] We also demonstrate that \emph{\textbf{improper}} fine-tuning can reduce a model's accuracy.
...
\headerbox{Links}{name=links,column=0,below=s-mac}{
	...
	\begin{minipage}[H]{0.4\textwidth}
		\begin{figure}[H]
			\includegraphics[width=0.4\textwidth]{../img/arxiv_paper_qr_code.png}
			\vspace{-10pt}
	...
	\begin{minipage}[H]{0.4\textwidth}
		\begin{figure}[H]
			\includegraphics[width=0.4\textwidth]{../img/reproducible_research_qr_code.png}
			\vspace{-10pt}
...
\headerbox{The CLEVR \& CoGenT datasets}{name=datasets, column=1}{
	...
	\begin{figure}[H]
		...
		\caption{\textit{How many objects are either small cylinders or red things?}}
...
\begin{itemize}
	\compresslist
	\item[$\bullet$] Our experiments on \emph{zero-shot learning} show that both models have poor performance, in line with
	...
	\item[$\rightarrow$] \emph{Zero-shot learning} (CoGenT-A $\rightarrow$ CoGenT-B) remains an interesting problem to solve.
\end{itemize}

}

\headerbox{MAC drawbacks on CLEVR}{name=mac-failures, column=1, below=experiments}{
	\vspace{-10pt}
	\begin{figure}[H]
	...
	\begin{itemize}
		\compresslist
		\item[$\bullet$] Asked about the shape of the leftmost gray cylinder, the model correctly finds it, (cf. \emph{visual attention map}), and refers to it using its color (\emph{attention over the question words}).
		\item[$\bullet$] Yet, predicts the shape as \textbf{cube}, as it never saw \emph{gray cylinders} during training, but saw \emph{gray cubes}.
		\item[$\rightarrow$] Indicate that MAC does not separate shape from color, but has a better understanding of colors (as found the object by its color).
	\end{itemize}
}
