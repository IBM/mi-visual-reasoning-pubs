...
\begin{itemize}
	...
	\item[$\bullet$] We evaluate the MAC model and the simplified variant on CLEVR \& CoGenT, and show that, \emph{\textbf{transfer learning with fine-tuning}} results in a 15 point increase in accuracy, matching the state of the art.
...
\headerbox{Simplified MAC Model (S-MAC)}{name=s-mac, column=0, below=mac-model}{
	...
	\begin{itemize}
		...
		\item[$\bullet$] We assume that dimension-preserving linear layers are invertible so as to avoid information loss. \vm{To reformulate?}
    \end{itemize}
...
\headerbox{The CLEVR \& CoGenT datasets}{name=datasets, column=1}{
	...
	\begin{figure}[H]
		...
		\caption{\textit{How many objects are either small cylinders or red things?} -- Answer: 5.}
